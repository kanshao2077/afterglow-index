...
当前数据来自 9 个来源族：Runway AI Film Festival、Project Odyssey、Reply AI Film Festival、HKUST AI Film Festival、AI Film Festival Japan、IFFI Cinema AI Hackathon、Bilibili 热门 AIGC、抖音出圈 AIGC 和 AI Film Landmarks。它们覆盖官方奖项页、创作者原发页、公开视频页与公开热度记录，并按 2026 年 8 月 1 日起的档案周期回填。获奖年份、作品发布时间和归档日期是三个独立字段，不会把旧作品伪装成当年新作。

线上地址：<https://www.kanshao2077.cn/afterglow-index/>
...
- 页面默认显示中文，可通过 `中 / EN` 切换并分享带 `?lang=en` 的英文链接；作品名和作者保留原文，简介随语言切换。
- 页面按 `类型 / 工具 / 来源 / 信号 / 时间` 筛选，并支持搜索、空状态、深链接和作品详情。
- 列表封面统一为 16:9；详情图完整显示，不裁掉竖版原作的关键构图。
...
- 最近 7 日目标约 7 件，通常在 5—9 件之间浮动；这是目标而非发布硬上限，质量不足时不硬凑，集中出现高质量作品时也不删档凑数。
...
机器筛选刻意保持透明：官方奖项、作者署名、作品描述、原作链接、可用封面、公开热度和行业记录组成信号分。它只决定「是否进入机器发现池」，不会冒充审美判断。Bilibili 适配器会读取公开热榜并用关键词与播放门槛找候选，同时保留少量已核验的热门作品入口，避免热榜轮换后丢失代表作。抖音当前使用带采集日期的可追溯热度快照，不宣称是稳定的匿名全站热榜；小红书因缺少稳定公开接口，暂不进入 MVP。
